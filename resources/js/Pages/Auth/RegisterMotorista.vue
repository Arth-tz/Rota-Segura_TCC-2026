...
const cnhNumeroError = computed(() => {
    if (!form.cnh_numero) return ''
    return form.cnh_numero.replace(/\D/g, '').length === 11 ? '' : 'Número da CNH deve ter 11 dígitos'
})
const cnhCategoriaError = computed(() => {
    if (!form.cnh_categoria) return ''
    return ['D', 'E'].includes(form.cnh_categoria) ? '' : 'Categoria deve ser D ou E'
})
const cnhValidadeError = computed(() => {
    if (!form.cnh_validade) return ''
    const hoje = new Date()
    hoje.setHours(0, 0, 0, 0)
    return new Date(form.cnh_validade) > hoje ? '' : 'Sua CNH está vencida'
})
    ...
    <div class="reg-root">
        ...
        <aside class="reg-aside" :class="{ 'reg-aside--in': pageVisible }">
            ...
            <div class="reg-aside__body">
                <div class="reg-aside__tag">Para motoristas</div>
                <h2 class="reg-aside__title">Ofereça um transporte escolar seguro e de qualidade</h2>
                <p class="reg-aside__sub">Cadastre seus dados, gerencie seus alunos e apareça para famílias da sua região.</p>
                <div class="reg-aside__perks">
                    <div class="reg-aside__perk">
                        <div class="reg-aside__perk-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="reg-perk-svg"><path stroke-linecap="round" stroke-linejoin="round" d="M9 6.75V15m6-6v8.25m.503 3.498l4.875-2.437c.381-.19.622-.58.622-1.006V4.82c0-.836-.88-1.38-1.628-1.006l-3.869 1.934c-.317.159-.69.159-1.006 0L9.503 3.252a1.125 1.125 0 00-1.006 0L3.622 5.689C3.24 5.88 3 6.27 3 6.695V19.18c0 .836.88 1.38 1.628 1.006l3.869-1.934c.317-.159.69-.159 1.006 0l4.994 2.497c.317.158.69.158 1.006 0z"/></svg>
                        </div>
                        <span>Gerencie alunos e rotas</span>
                    </div>
                    <div class="reg-aside__perk">
                        <div class="reg-aside__perk-icon">
                        ...
                        <span>Documentação verificada e visível</span>
                    </div>
                    <div class="reg-aside__perk">
                        <div class="reg-aside__perk-icon">
                        ...
                        <span>Apareça para famílias da sua região</span>
                    </div>
                    <div class="reg-aside__perk">
                        <div class="reg-aside__perk-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="reg-perk-svg"><path stroke-linecap="round" stroke-linejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z"/></svg>
                        </div>
        ...
        <main class="reg-main" :class="{ 'reg-main--in': pageVisible }">
            <div class="reg-form-wrap">
                ...
                <div class="reg-heading">
                    ...
                    <p class="reg-heading__sub">Cadastre-se como motorista e expanda sua rota.</p>
                </div>
                ...
                <form @submit.prevent="submit" class="reg-form">
                    ...
                    <div class="reg-section">
                        <div class="reg-section__label">
                            <svg xmlns="http://www.w3.org/2000/svg" class="reg-section__icon" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z"/></svg>
                            Dados pessoais
                        </div>
                    </div>
                    ...
                    <div class="reg-field">
                        ...
                        <div class="reg-input-wrap">
                            ...
                            <input v-model="form.email" type="email" placeholder="motorista@email.com"
                    ...
                    <div class="reg-row">
                        <div class="reg-field">
                            <label class="reg-label">Nº CNH</label>
                            <input v-model="form.cnh_numero" type="text" placeholder="00000000000"
                                maxlength="11"
                                class="reg-input" :class="{ 'reg-input--error': form.errors.cnh_numero || cnhNumeroError }" />
                            <p v-if="form.errors.cnh_numero || cnhNumeroError" class="reg-error">{{ form.errors.cnh_numero || cnhNumeroError }}</p>
                        </div>
                        <div class="reg-field">
                            <label class="reg-label">Categoria</label>
                            <div class="reg-input-wrap">
                                <svg xmlns="http://www.w3.org/2000/svg" class="reg-input-icon" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M8.25 15L12 18.75 15.75 15m-7.5-6L12 5.25 15.75 9"/></svg>
                                <select v-model="form.cnh_categoria"
                                    class="reg-input reg-input--icon reg-select"
                                    :class="{ 'reg-input--error': form.errors.cnh_categoria || cnhCategoriaError }">
                                    <option value="" disabled>Selecione</option>
                                    <option value="D">D</option>
                                    <option value="E">E</option>
                                </select>
                            </div>
                            <p v-if="form.errors.cnh_categoria || cnhCategoriaError" class="reg-error">{{ form.errors.cnh_categoria || cnhCategoriaError }}</p>
                        </div>
                    </div>

                    <!-- Validade CNH -->
                    <div class="reg-field">
                        <label class="reg-label">Validade da CNH</label>
                        <div class="reg-input-wrap">
                            <svg xmlns="http://www.w3.org/2000/svg" class="reg-input-icon" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5"/></svg>
                            <input v-model="form.cnh_validade" type="date"
                                class="reg-input reg-input--icon"
                                :class="{ 'reg-input--error': form.errors.cnh_validade || cnhValidadeError }" />
                        </div>
                        <p v-if="form.errors.cnh_validade || cnhValidadeError" class="reg-error">{{ form.errors.cnh_validade || cnhValidadeError }}</p>
                    </div>
                    ...
                    <!-- Submit -->
                    <button type="submit" :disabled="form.processing" class="reg-submit">
...
.reg-aside {
    display: none;
    width: 38%; min-width: 340px; max-width: 460px;
...
@media (min-width: 480px) { .reg-main { padding: 36px 32px 72px; } }
@media (min-width: 768px) { .reg-main { padding: 48px 56px 72px; } }
@media (min-width: 1024px) { .reg-main { padding: 52px 64px 72px; display: flex; align-items: flex-start; justify-content: center; } }
.reg-main--in { opacity: 1; transform: translateY(0); }

.reg-form-wrap { width: 100%; max-width: 600px; }
@media (min-width: 1024px) { .reg-form-wrap { margin: 0 auto; } }
...
/* Heading */
.reg-heading { margin-bottom: 22px; }
.reg-heading__title {
    font-family: 'Nunito', sans-serif;
...
/* Section dividers */
.reg-section { margin: 6px 0 2px; }
.reg-section__label {
    display: flex; align-items: center; gap: 7px;
    font-size: 0.72rem; font-weight: 700; color: #2563eb;
    letter-spacing: 0.07em; text-transform: uppercase;
    padding-bottom: 8px; border-bottom: 1.5px solid #dbeafe;
}
.reg-section__icon { width: 14px; height: 14px; flex-shrink: 0; }

/* Form */
.reg-form { display: flex; flex-direction: column; gap: 14px; }
@media (min-width: 480px) { .reg-form { gap: 15px; } }
...
.reg-select:focus { border-color: #2563eb; box-shadow: 0 0 0 3px rgba(37,99,235,0.1); background: #fafcff; }

/* Date input */
input[type="date"].reg-input { cursor: pointer; }
input[type="date"].reg-input::-webkit-calendar-picker-indicator {
    opacity: 0.5; cursor: pointer;
    filter: invert(40%) sepia(60%) saturate(400%) hue-rotate(195deg);
}
