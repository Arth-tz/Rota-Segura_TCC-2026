...
const TIPO_RESPONSAVEL = { pai: 'Pai', mae: 'Mãe', avo: 'Avô/Avó', responsavel_legal: 'Resp. Legal', representante_legal: 'Rep. Legal' }
...
const statusSolicitacao = {
    ...
    recusada:  { label: 'Recusada',  classes: 'bg-red-50 text-red-500 border-red-100' },
    cancelada: { label: 'Cancelada', classes: 'bg-slate-100 text-slate-500 border-slate-200' },
}

function aceitarSolicitacao(id) {
    router.post(route('admin.solicitacoes.aceitar', id))
}
...
function confirmarRecusa() {
    router.post(route('admin.solicitacoes.recusar', modalRecusar.value.id_solicitacao), {
        motivo: motivoRecusa.value,
    }, { onSuccess: () => { modalRecusar.value = null } })
}
...
<template>
            ...
            <div v-else class="space-y-3">
                ...
                <div v-for="s in solicitacoes" :key="s.id_solicitacao"
                    ...
                    <div class="flex flex-col items-end gap-2 shrink-0">
                        ...
                        <div v-if="s.status === 'pendente'" class="flex gap-1.5 mt-1">
                            <button @click="aceitarSolicitacao(s.id_solicitacao)"
                                class="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold transition">
                                <CheckCircleIcon class="w-3.5 h-3.5" /> Aceitar
                            </button>
                            <button @click="abrirRecusa(s)"
                                class="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-red-200 text-red-600 hover:bg-red-50 text-xs font-semibold transition">
                                <XCircleIcon class="w-3.5 h-3.5" /> Recusar
                            </button>
                        </div>
                    </div>
                </div>
    ...
    <!-- Modal recusa de solicitação -->
    <Teleport to="body">
        <Transition name="fade">
            <div v-if="modalRecusar"
                class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm"
                @click.self="modalRecusar = null">
                <Transition name="pop">
                    <div v-if="modalRecusar" class="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl border border-slate-100">
                        <h3 class="text-base font-bold text-slate-900 mb-1">Recusar solicitação</h3>
                        <p class="text-sm text-slate-500 mb-4">Passageiro: {{ modalRecusar.passageiro }}</p>

                        <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">
                            Motivo <span class="text-slate-400 normal-case font-normal">(opcional)</span>
                        </label>
                        <textarea v-model="motivoRecusa" rows="3" placeholder="Descreva o motivo da recusa..."
                        ...
                        <div class="flex gap-2 mt-4">
                            ...
                            <button @click="modalRecusar = null"
