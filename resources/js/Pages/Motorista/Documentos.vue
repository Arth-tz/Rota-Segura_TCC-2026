...
function enviarRenach() {
    if (!inputRenach.value?.files[0]) return
    formRenach.arquivo = inputRenach.value.files[0]
    formRenach.post(route('motorista.documentos.upload', 'renach'), {
        forceFormData: true,
        onSuccess: () => { formRenach.reset(); if (inputRenach.value) inputRenach.value.value = '' },
    })
}
        ...
        <main class="max-w-2xl mx-auto px-4 py-6 space-y-5 pb-12">
            ...
            <section class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                <div class="flex items-center gap-3 px-5 py-4 border-b border-slate-100 bg-slate-50/60">
                    ...
                    <div class="flex-1 min-w-0">
                        <h2 class="text-base font-bold text-slate-800">Extrato RENACH (Infrações)</h2>
                        <p class="text-xs text-slate-500">Histórico de infrações — opcional, recomendado (JPG, PNG ou PDF, até 5 MB)</p>
                    </div>
                    <CheckCircleIcon v-if="motorista.renach_url" class="w-5 h-5 text-emerald-500 shrink-0" />
                </div>

                <div class="px-5 py-4 space-y-3">
                    <p class="text-xs text-slate-500">
                        Emitido pelo <strong class="text-slate-700">DETRAN do seu estado</strong> ou pelo app
                        <strong class="text-slate-700">gov.br → Carteira Digital de Trânsito</strong> → "Extrato RENACH".
                        Visível apenas pelo administrador; responsáveis veem apenas "Ficha limpa: Sim/Não".
                    </p>

                    <div v-if="motorista.renach_url"
                        class="flex items-center gap-2 rounded-xl border border-emerald-100 bg-emerald-50 px-3 py-2.5">
                        <DocumentTextIcon class="w-4 h-4 text-emerald-600 shrink-0" />
                        <span class="text-xs text-emerald-700 flex-1">Arquivo enviado</span>
                        <a :href="motorista.renach_url" target="_blank" rel="noopener noreferrer"
                            class="flex items-center gap-1 text-xs font-semibold text-emerald-700 hover:underline">
                            <ArrowTopRightOnSquareIcon class="w-3 h-3" />
                            Visualizar
                        </a>
                    ...
                    <form @submit.prevent="enviarRenach" class="flex gap-2">
                        <input ref="inputRenach" type="file" accept=".jpg,.jpeg,.png,.webp,.pdf"
                            class="flex-1 text-xs text-slate-600 file:mr-3 file:text-xs file:font-semibold
                                   file:rounded-lg file:border-0 file:bg-slate-100 file:text-slate-700
                                   file:px-3 file:py-2 hover:file:bg-slate-200 file:transition
                                   border border-slate-200 rounded-xl px-2 py-2 bg-white" />
                        <button type="submit" :disabled="formRenach.processing"
                            class="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-600 hover:bg-slate-700
                                   disabled:opacity-60 text-white text-xs font-semibold transition shrink-0">
                            <ArrowUpTrayIcon class="w-3.5 h-3.5" />
                            {{ formRenach.processing ? 'Enviando…' : (motorista.renach_url ? 'Substituir' : 'Enviar') }}
                        </button>
                    </form>
                    <p v-if="formRenach.errors.arquivo" class="text-red-500 text-xs">{{ formRenach.errors.arquivo }}</p>
                </div>
            </section>
