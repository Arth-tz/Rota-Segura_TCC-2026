    ...
    <div class="max-w-lg space-y-4">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div class="flex items-center gap-4 px-5 py-4 border-b border-blue-100 bg-blue-50/60">
                <div class="h-12 w-12 rounded-full overflow-hidden bg-blue-600 flex items-center justify-center shrink-0">
                    <img v-if="usuario?.foto_url" :src="usuario.foto_url" class="w-full h-full object-cover" alt="" />
                    <span v-else class="text-white font-bold text-lg">{{ usuario?.nome?.charAt(0)?.toUpperCase() ?? '?' }}</span>
                </div>
                <div>
