...
import { TruckIcon, UserGroupIcon, MapIcon, PlusIcon, PencilSquareIcon, CameraIcon, ExclamationCircleIcon, CheckCircleIcon, XCircleIcon, ArrowRightIcon, ShieldCheckIcon, EyeIcon, EyeSlashIcon, ClipboardDocumentIcon, ClipboardDocumentCheckIcon, XMarkIcon, ChevronDownIcon } from '@heroicons/vue/24/outline'
...
// ── Checklist: colapsar / fechar ─────────────────────────────────────────────
const checklistColapsado = ref(localStorage.getItem('rota_checklist_colapsado') === '1')
const checklistFechado   = ref(sessionStorage.getItem('rota_checklist_fechado') === '1')

function toggleChecklist() {
    checklistColapsado.value = !checklistColapsado.value
    localStorage.setItem('rota_checklist_colapsado', checklistColapsado.value ? '1' : '0')
}

function fecharChecklist() {
    checklistFechado.value = true
    sessionStorage.setItem('rota_checklist_fechado', '1')
}
...
<template>
    <div class="space-y-4">
        ...
        <!-- ── CHECKLIST DE ONBOARDING ────────────────────────────────────── -->
        <div v-if="mostrarChecklist && !checklistFechado"
            ...
            <button type="button"
                class="w-full flex items-center justify-between gap-3 px-5 py-4 text-left border-b transition-colors"
                :class="tudo_pronto ? 'border-slate-100 bg-slate-50 hover:bg-slate-100/70' : 'border-amber-100 bg-amber-50/70 hover:bg-amber-100/60'"
                ...
                <div class="flex items-center gap-2 shrink-0">
                    ...
                    <ChevronDownIcon
                        ...
                        :class="checklistColapsado ? '-rotate-90' : ''" />
                    <button type="button"
                        class="w-6 h-6 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-200 transition-colors"
                        title="Fechar"
                        @click.stop="fecharChecklist">
            ...
            <!-- Itens (colapsaveis) -->
            <div class="checklist-body" :class="checklistColapsado ? 'checklist-colapsado' : ''">
...
.checklist-body.checklist-colapsado {
...
@media (prefers-reduced-motion: reduce) {
    .checklist-body { transition: none; }
}
</style>
